...
class UploadrTagLib {
    ...
    def add = { attrs, body ->
        ...
        def additionalParams    = (attrs.containsKey('params') ? attrs.get('params') : [:])
        ...
        if (attrs.get('controller')) {
            ...
            if (attrs.get('action')) {
                ...
                if (attrs.get('plugin')) {
                    uri = createLink(plugin: attrs.plugin, controller: attrs.controller, action: attrs.action, params: additionalParams)
                } else {
                    uri = createLink(controller: attrs.controller, action: attrs.action, params: additionalParams)
                }
            } else {
                // got a plugin attribute?
                if (attrs.get('plugin')) {
                    uri = createLink(plugin: attrs.plugin, controller: attrs.controller, params: additionalParams)
                } else {
                    uri = createLink(controller: attrs.controller, params: additionalParams)
                }
            }
        } else {
            // use default controller for handeling file uploads
            uri = createLink(plugin: 'uploadr', controller: 'upload', action: 'handle', params: additionalParams)
        }
