...
EditPage.prototype.init = function() {
	...
	$("#linkDialog-searchField")
			.autocomplete(
					{
						minLength : 2,
						source : this.options.pageTitleAutoCompleteUrl,
...
EditPage.prototype.reloadImageGallery = function() {
	...
	$('#imageDialog-gallery').load(
			this.options.imageGalleryUrl,
			{
				pageId : this.pageId,
				ts : new Date().getTime()
			},
			function() {
				$('#imageDialog-gallery .entry').each(
						function(index, el) {
							...
							$('.image', el).click(
									function() {
										self.jqBody.insertAtCaret('![' + name
												+ '](' + self.pageId + '/'
												+ name + ')');
...
EditPage.prototype.deleteAttachment = function(name) {
	...
	if (confirm('Attachment "' + name + '" wirklich löschen?')) {
		$.post(this.options.deleteAttachmentUrl, {
			pageId : this.pageId,
			filename : name
		}, function(a, b, c) {
